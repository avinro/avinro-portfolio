...
<FlowSplit>
  <FlowColumn title="Before UMA" subtitle="Manual, monthly, scattered" tone="negative" badgeLabel="Status quo">
  ...
  <FlowColumn title="With UMA" subtitle="Capture in seconds, see it the same day" tone="positive" badgeLabel="Automated">
...
<StateGrid columns={3}>
  ...
  <StateItem icon="flask-conical" label="No research infra" hint="Validated through founder + peer chats" />
  <StateItem icon="brain" label="AI-assisted dev, first time" hint="Cursor + Claude, learned on the job" />
  <StateItem icon="hourglass" label="5–10s extraction latency" hint="Had to design for asynchronous feedback" />
  <StateItem icon="coins" label="Funding uncertain" hint="Built toward a launch that never closed" />
</StateGrid>
...
<PrincipleGrid>
  <PrincipleItem icon="accessibility" title="Accessibility over comprehensiveness" body="Remove every barrier to adoption. A restaurant owner should not learn a new app or change their behaviour — meet them where they already are: WhatsApp." />
  <PrincipleItem icon="layers" title="Depth on demand" body="WhatsApp handles the daily upload and quick questions. The web app is where owners go to dig into data, understand trends, and export for advisors. Two interfaces, one product." />
  <PrincipleItem icon="target" title="Build what restaurants actually need" body="Reject document-management views, processing queues, and approval workflows. Restaurants care about one thing — am I making money? — so show that first and let everything else drill from it." />
</PrincipleGrid>
...
<BranchTree icon="user" label="Restaurant owner" hint="One user, three cadences, one Supabase truth">
...
<StateGrid columns={4}>
  <StateItem icon="upload" label="Uploaded" hint="Waiting for the extraction worker" tone="neutral" />
  <StateItem icon="loader" label="Processing" hint="AI is reading the receipt" tone="warning" />
  <StateItem icon="alert-triangle" label="Needs review" hint="Low confidence — user verifies" tone="warning" />
  <StateItem icon="check-circle-2" label="Processed" hint="Extracted and categorized" tone="accent" />
...
<Bar value={100} label="Design tokens shared between Figma and Tailwind config" caption="Single source of truth: Figma variables exported to JSON, fed directly into Tailwind config. Zero manual colour or spacing values in component files." />
