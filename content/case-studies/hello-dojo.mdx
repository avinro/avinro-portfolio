...
<StateGrid columns={4}>
  <StateItem icon="smartphone" label="Customer App" hint="iOS — book a ride, table, yacht or VIP" tone="accent" />
  <StateItem icon="car" label="Driver App" hint="iOS — accept requests, navigate, complete" tone="accent" />
  <StateItem icon="map" label="Fleet Portal" hint="Web — vehicles, drivers, compliance" tone="accent" />
  <StateItem icon="store" label="Vendor Portal" hint="Web — availability, bookings, demand" tone="accent" />
</StateGrid>
...
<PrincipleGrid>
  <PrincipleItem icon="mic" title="Voice Mode" body="Hold-to-talk, premium hands-free. Dojo speaks back and shows option cards beneath the avatar — same conversation continues whether you tap or speak." />
  <PrincipleItem icon="message-square" title="Chat Mode" body="Same flow as voice, written. For loud clubs, hearing accessibility, or just preference. Option cards drop in below each Dojo reply." />
  <PrincipleItem icon="layout-grid" title="UI Flow" body="For users who want to browse all 50 restaurants before deciding. Stepped, filterable, elevated visual treatment — still routes through the same booking engine." />
</PrincipleGrid>
...
<StateGrid columns={4}>
  <StateItem icon="car" label="Rideshare" hint="Apps, taxi ranks, WhatsApp groups" tone="negative" />
  <StateItem icon="wine" label="Dining" hint="Resy, calls, concierge favours" tone="negative" />
  <StateItem icon="sailboat" label="Yachts" hint="Brokers, deposits, side-channel WhatsApp" tone="negative" />
  <StateItem icon="ticket" label="Nightlife & events" hint="VIP lists, door staff, who-you-know" tone="negative" />
</StateGrid>
...
<PrincipleGrid>
  <PrincipleItem icon="users" title="User problem" body="Tourists and locals need rides, dinners, yachts, tables — but the system runs on insider connections, scattered apps, and language-blind concierges." tone="warning" />
  <PrincipleItem icon="building-2" title="Business problem" body="Capture transaction volume across rideshare, dining and events before competitors land. Half-baked or fragmented = failure in an image-conscious market." tone="warning" />
  <PrincipleItem icon="puzzle" title="Product problem" body="Unify radically different service categories (instant rides, advance bookings, payments, logistics) under one experience — without overwhelming or fragmenting." tone="warning" />
</PrincipleGrid>
...
<StateGrid columns={4}>
  ...
  <StateItem icon="flask-conical" label="No prior validation" hint="Only rideshare demand was confirmed" />
  <StateItem icon="network" label="Parallel system complexity" hint="Decisions in one product rippled into all 4" />
  <StateItem icon="layers" label="Three modes at once" hint="Voice + Chat + UI Flow, all coherent" />
  <StateItem icon="bot" label="Novel AI-driven UX" hint="Latency, recognition, fallback all open" />
  <StateItem icon="shield-check" label="Safety-critical ops" hint="Driver app + fleet portal on the road" />
  <StateItem icon="palette" label="One DS, many contexts" hint="Apps + web, consumer + operational" />
...
<PrincipleGrid>
  <PrincipleItem icon="bot" title="Agentic by default, UI as support" body="Voice and Chat are the primary path. Users feel they are talking to a capable concierge. UI cards support the conversation — they never replace it." />
  <PrincipleItem icon="route" title="Voice-first DNA, even in UI Flow" body="The structured browse path still feels premium and agentic — same Dojo voice, same booking logic — just a different visual treatment." />
  <PrincipleItem icon="cog" title="Unified service taxonomy" body="Rideshare, dining, yachts and events are not separate products. They are service categories accessed through one interface, one mental model." />
  <PrincipleItem icon="radio" title="Real-time operational transparency" body="Drivers see demand. Fleet managers see deployment. Venues see availability. Transparency reduces friction across all four sides of the platform." />
  <PrincipleItem icon="link" title="User continuity and reduced friction" body="Book a restaurant, then need a ride — the system remembers you, pre-fills, opens the right account. Switching costs measured in seconds." />
  <PrincipleItem icon="flag" title="Scope discipline through prioritization" body="Rideshare first, then expansion. Push back on big-bang launches; build a validated core before stretching across categories." />
</PrincipleGrid>
...
<FlowSplit>
  <FlowColumn title="Option A — Uber clone" subtitle="Map-first, tap-to-request, AI layered on top" tone="negative" badgeLabel="Discarded">
    ...
    <FlowItem icon="alert-triangle" label="Zero differentiation" hint="Ignores Ibiza's environment" />
  </FlowColumn>
  <FlowColumn title="Option B — SaaS portal" subtitle="Forms + menus, AI assistant on the side" tone="negative" badgeLabel="Discarded">
    <FlowItem icon="clipboard-list" label="Browse and form-fill" />
    <FlowItem icon="message-square" label="AI as helpful chat layer" />
    <FlowItem icon="alert-triangle" label="Defeats concierge positioning" hint="Just another Resy clone" />
  </FlowColumn>
  <FlowColumn title="Option C — Agentic, three modes" subtitle="Dojo as primary across Voice / Chat / UI" tone="positive" badgeLabel="Chosen">
    ...
    <FlowItem icon="check-circle-2" label="One Dojo, one booking engine" hint="Three doors, same room" />
  </FlowColumn>
</FlowSplit>
...
<FlowChain highlightLast>
  ...
  <FlowItem icon="eye" label="Review" hint="Loop until edge cases and a11y are clean" tone="warning" />
...
<StateGrid columns={4}>
  ...
  <StateItem icon="briefcase" label="CEO" hint="Rideshare validation, scope pressure, business calls" />
  <StateItem icon="palette" label="DS lead (subcontract)" hint="Token automation, AI-readable Figma docs" />
...
<StateGrid columns={4}>
  ...
  <StateItem icon="shield-check" label="Compliance automation" hint="Renewal reminders, DB integrations" />
  <StateItem icon="bar-chart-2" label="Demand prediction" hint="ML on historical request patterns" />
  <StateItem icon="sparkles" label="Dojo personality depth" hint="Voice, humour, regional references" />
</StateGrid>
