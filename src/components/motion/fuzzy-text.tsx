...
import { Children, useEffect, useRef, useSyncExternalStore, type ReactNode } from "react";
...
export default function FuzzyText({
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      const fontSizeStr = typeof fontSize === "number" ? `${String(fontSize)}px` : fontSize;
      const fontString = `${String(fontWeight)} ${fontSizeStr} ${computedFontFamily}`;
      ...
      offCtx.font = `${String(fontWeight)} ${fontSizeStr} ${computedFontFamily}`;
      ...
      const rawLeft = metrics.actualBoundingBoxLeft;
      const leftExtent = Number.isFinite(rawLeft) ? rawLeft : 0;
      const actualRight =
      ...
      const textBoundingWidth = Math.ceil(
        letterSpacing !== 0 ? totalWidth : leftExtent + actualRight,
      );
      ...
      if (letterSpacing !== 0) {
        ...
        offCtx.fillText(text, xOffset - leftExtent, actualAscent);
      }
      ...
      dispose = cleanupListenersAndTimers;
      // eslint-disable-next-line @typescript-eslint/no-unnecessary-condition -- effect cleanup can set isCancelled while this async init runs
      if (isCancelled) {
