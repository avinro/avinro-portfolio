...
const createTouchTexture = () => {
  ...
  const drawPoint = (p: {
    ...
    intensity *= p.force;
    const r = String(Math.round(((p.vx + 1) / 2) * 255));
    const g = String(Math.round(((p.vy + 1) / 2) * 255));
    const b = String(Math.round(intensity * 255));
    const color = `${r}, ${g}, ${b}`;
    const offset = size * 5;
    ctx.shadowOffsetX = offset;
    ctx.shadowOffsetY = offset;
    ctx.shadowBlur = radius;
    ctx.shadowColor = `rgba(${color},${String(0.22 * intensity)})`;
    ctx.beginPath();
  ...
  useEffect(() => {
    ...
    if (mustReinit) {
      ...
      const setSize = () => {
        ...
        if (cur?.composer)
          cur.composer.setSize(renderer.domElement.width, renderer.domElement.height);
